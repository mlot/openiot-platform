...
public class GnuHealthConfiguration {
	 ...
	 */
	public GnuHealthData getGnuHealthData() throws OpenIoTException {
	 ...
	 * @throws com.openiot.spi.OpenIoTException
	 */
	protected void login(WebClient client) throws OpenIoTException {
		JsonCall login =
		...
		try {
			...
			throw new OpenIoTException("Unable to parse login response.", e);
		} catch (IOException e) {
			throw new OpenIoTException("Unable to read login response.", e);
		}
		JsonNode resultJson = json.get("result");
		if (resultJson.isBoolean()) {
			throw new OpenIoTException("GNU Health login failed.");
		}
		if (!resultJson.isArray()) {
			throw new OpenIoTException("Unexpected login response from GNU Health: "
					+ resultJson.toString());
		}
