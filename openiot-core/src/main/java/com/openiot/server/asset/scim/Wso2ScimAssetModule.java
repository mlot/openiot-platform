...
public class Wso2ScimAssetModule extends LifecycleComponent implements IAssetModule<PersonAsset> {
	 ...
	 */
	public PersonAsset getAssetById(String id) throws OpenIoTException {
	 ...
	 */
	public List<PersonAsset> search(String criteria) throws OpenIoTException {
	...
	protected ICommandResponse cacheAssetData() throws OpenIoTException {
		...
		try {
			...
			throw new OpenIoTException("Unable to parse asset response.", e);
		} catch (IOException e) {
			throw new OpenIoTException("Unable to read asset response.", e);
		}
	 ...
	 * @return
	 */
	protected PersonAsset parse(JsonNode resource) throws OpenIoTException {
		PersonAsset asset = new PersonAsset();
		JsonNode id = resource.get(IScimFields.ID);
		if (id == null) {
			throw new OpenIoTException("SCIM resource does not have an id.");
		}
