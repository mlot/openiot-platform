...
public class DeviceMarshalHelper {
	 ...
	 */
	public Device convert(IDevice source, IAssetModuleManager manager) throws OpenIoTException {
		...
		for (IDeviceElementMapping mapping : source.getDeviceElementMappings()) {
			...
			if (isIncludeNested()) {
				...
				cnvMapping.setDevice(getNestedHelper().convert(device,
						OpenIoT.getServer().getAssetModuleManager()));
			}
		...
		if (source.getSpecificationToken() != null) {
			...
			if (spec == null) {
				throw new OpenIoTException("Device references non-existent specification.");
			}
			if (includeSpecification) {
				...
				if (asset != null) {
					...
					throw new OpenIoTException("Specification references non-existent asset.");
				}
			}
		}
			...
			if (includeAssignment) {
				try {
					IDeviceAssignment assignment =
							OpenIoT.getServer().getDeviceManagement().getCurrentDeviceAssignment(source);
					if (assignment == null) {
						throw new OpenIoTException("Device contains an invalid assignment reference.");
					}
		...
		if (source.getSiteToken() != null) {
			if (includeSite) {
				ISite site =
						OpenIoT.getServer().getDeviceManagement().getSiteByToken(source.getSiteToken());
				if (site == null) {
					throw new OpenIoTException("Device contains an invalid site reference.");
				}
