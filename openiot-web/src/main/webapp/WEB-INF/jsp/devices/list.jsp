...
<script>
    ...
    function checkHealth(assignmentToken){

        if (assignmentToken) {

            var dsUrl = "${pageContext.request.contextPath}/api/assignments/"+assignmentToken+"/alerts?page=1&pageSize=100";
            dsUrl += "&startDate=" + moment().subtract(5, 'minutes').toISOString();  // 5 minutes window

            $.getJSON(dsUrl, onGetSuccess);
        }

        function onGetSuccess(data, status, xhr){
            //console.log(data);
            updateHealthStatus(assignmentToken, data);
        }

        function updateHealthStatus(token, alerts){
            var status = {};
            var alertCount = alerts.results.length;
            var imageContainer = $("#status"+token);
            imageContainer.attr("title", alertCount+" ticks in 5 minutes");
            imageContainer.attr("src","/img/"+getHealthIcon(alertCount));
        }
    }

    function getHealthIcon(alertCount){
        if (alertCount >=20){
            return "signal-5.png";
        }
        if (alertCount >=15){
			...
			schema : {
				...
				parse : function(response) {
					$.each(response.results, function(index, item) {
                        ...
                        if (item.assignment){
                            checkHealth(item.assignment.token);
                        }
					});
					return response;
				}
