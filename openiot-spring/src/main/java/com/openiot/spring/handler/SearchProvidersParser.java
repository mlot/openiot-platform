...
import com.openiot.server.search.SearchProviderManager;
import com.openiot.solr.OpenIoTSolrConfiguration;
import com.openiot.solr.search.SolrSearchProvider;
...
public class SearchProvidersParser extends AbstractBeanDefinitionParser {
	...
	protected AbstractBeanDefinition parseInternal(Element element, ParserContext context) {
		...
		context.getRegistry().registerBeanDefinition(OpenIoTServerBeans.BEAN_SEARCH_PROVIDER_MANAGER,
		...
		for (Element child : children) {
			if (!IConfigurationElements.OPENIOT_COMMUNITY_NS.equals(child.getNamespaceURI())) {
				...
				if (nested != null) {
					...
					throw new RuntimeException("Invalid nested element found in 'search-providers' section: "
							+ child.toString());
				}
			}
	...
	protected AbstractBeanDefinition parseSolrSearchProvider(Element element, ParserContext context) {
		...
		provider.addPropertyReference("solr", OpenIoTSolrConfiguration.SOLR_CONFIGURATION_BEAN);
