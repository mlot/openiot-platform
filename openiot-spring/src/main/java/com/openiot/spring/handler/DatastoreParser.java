...
import com.openiot.hazelcast.HazelcastDistributedCacheProvider;
import com.openiot.hazelcast.OpenIoTHazelcastConfiguration;
import com.openiot.hbase.DefaultHBaseClient;
import com.openiot.hbase.device.HBaseDeviceManagement;
import com.openiot.hbase.user.HBaseUserManagement;
import com.openiot.mongodb.DockerMongoClient;
import com.openiot.mongodb.OpenIoTMongoClient;
import com.openiot.mongodb.device.MongoDeviceManagement;
import com.openiot.mongodb.user.MongoUserManagement;
import com.openiot.server.OpenIoTServerBeans;
import com.openiot.server.device.DefaultDeviceModelInitializer;
import com.openiot.server.user.DefaultUserModelInitializer;
...
/**
 * Parses configuration data for the OpenIoT datastore section.
 * 
 * @author Derek
 */
public class DatastoreParser extends AbstractBeanDefinitionParser {
	...
	protected void parseMongoDatasource(Element element, ParserContext context) {
		...
		BeanDefinitionBuilder client =
				docker ? BeanDefinitionBuilder.rootBeanDefinition(DockerMongoClient.class)
						: BeanDefinitionBuilder.rootBeanDefinition(OpenIoTMongoClient.class);
		...
		BeanDefinitionBuilder dm =
				BeanDefinitionBuilder.rootBeanDefinition(MongoDeviceManagement.class);
		dm.addPropertyReference("mongoClient", "mongo");
		context.getRegistry().registerBeanDefinition(OpenIoTServerBeans.BEAN_DEVICE_MANAGEMENT,
				dm.getBeanDefinition());

		// Register Mongo user management implementation.
		BeanDefinitionBuilder um =
				BeanDefinitionBuilder.rootBeanDefinition(MongoUserManagement.class);
		um.addPropertyReference("mongoClient", "mongo");
		context.getRegistry().registerBeanDefinition(OpenIoTServerBeans.BEAN_USER_MANAGEMENT,
	...
	protected void parseHBaseDatasource(Element element, ParserContext context) {
		// Register client bean.
		BeanDefinitionBuilder client =
				BeanDefinitionBuilder.rootBeanDefinition(DefaultHBaseClient.class);
		...
		BeanDefinitionBuilder dm =
				BeanDefinitionBuilder.rootBeanDefinition(HBaseDeviceManagement.class);
		...
		BeanDefinitionBuilder um =
				BeanDefinitionBuilder.rootBeanDefinition(HBaseUserManagement.class);
	...
	protected void parseEHCacheDeviceManagementCache(Element element, ParserContext context) {
		BeanDefinitionBuilder cache =
				BeanDefinitionBuilder.rootBeanDefinition("com.openiot.ehcache.DeviceManagementCacheProvider");
		...
		context.getRegistry().registerBeanDefinition(
				OpenIoTServerBeans.BEAN_DEVICE_MANAGEMENT_CACHE_PROVIDER, cache.getBeanDefinition());
	}
	...
	protected void parseDefaultDeviceModelInitializer(Element element, ParserContext context) {
		...
		context.getRegistry().registerBeanDefinition(OpenIoTServerBeans.BEAN_DEVICE_MODEL_INITIALIZER,
	...
	protected void parseDefaultUserModelInitializer(Element element, ParserContext context) {
		...
		context.getRegistry().registerBeanDefinition(OpenIoTServerBeans.BEAN_USER_MODEL_INITIALIZER,
				init.getBeanDefinition());
	}
